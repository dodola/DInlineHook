...
void init_entries(JNIEnv *env) {
    ...
    if (api_level < 23) {
        ...
        suspendAll = reinterpret_cast<void (*)(ScopedSuspendAll *, char *)>(fake_dlsym(handle,
...
jboolean epic_compile(JNIEnv *env, jclass, jobject method, jlong self) {
    ...
    bool ret = jit_compile_method_(jit_compiler_handle_, reinterpret_cast<void *>(art_method),
